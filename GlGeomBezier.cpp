/*
* GlGeomBezier.cpp - Version 1.1 - November 13, 2020
*
...
// Render the n patches as triangles, starting with patch number i
void GlGeomBezier::RenderPatches(int i, int n)
{
    assert(i >= 0 && i + n <= numPatches);
    if (n == 0) {
        return;
    }
    PreRender();
    ...
    int numTriangles = 0;
    for (int j = i; j < i + n; j++ ) {
        numTriangles += GetNumTrisInPatch(j);
    }
   
    GlGeomBase::RenderEBO(GL_TRIANGLES, 3 * numTriangles, patchEboIndex);
}
